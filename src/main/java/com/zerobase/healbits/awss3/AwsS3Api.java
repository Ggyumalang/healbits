...
import com.zerobase.healbits.util.FileUtil;
...
import static com.zerobase.healbits.type.ErrorCode.FAILED_UPLOAD_FILE;
...
public class AwsS3Api {
    ...
    public String uploadImage(
        ...
        String filePath = FileUtil.createFilePath(challengeCategory, challengeName, email);
        ...
        try (InputStream inputStream = multipartFile.getInputStream()) {
            amazonS3Client.putObject(new PutObjectRequest(bucket, filePath + fileName, inputStream, objectMetadata)
                    .withCannedAcl(CannedAccessControlList.PublicRead));
        } catch (IOException e) {
            throw new HealBitsException(FAILED_UPLOAD_FILE);
        }
        log.info("uploadImage finished " + LocalDateTime.now() + " ImagePath : " + filePath + fileName);
